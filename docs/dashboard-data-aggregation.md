...
更新内容:
- 月度趋势与年度累计改为按月份选源：存在公司月度 total 时使用该权威值，否则汇总订单级净回款；4 月只计月度总额一次，订单继续作为人员下钻。
...
- 公司级月度业绩：`scripts/create_revenue_monthly_tables.sql` 创建 `fact_revenue_channel_monthly`。同一月份存在 `total` 行时，KPI 与趋势使用该权威总额；`channel` 行只用于四渠道结构拆分，不与 total 或订单表再次相加。因忽略列可能包含额外收入，四渠道之和不要求等于实际营收总额。
...
- 本月目标和月趋势目标使用 `biz_target_monthly.target_amount_yuan`；年度目标汇总同表中的月目标。本次公司业绩工作簿不导入目标。

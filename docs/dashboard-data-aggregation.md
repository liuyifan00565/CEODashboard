...
更新内容: 业务月份口径和前端默认日期范围改为中国时区当前自然月；事实表尚未同步当前月时只影响实际值，不再把上一事实月显示为“本月”。
...
业务月份 `latestMonth` 使用中国时区当前自然月，因此 2026-07-08 访问时首页显示 2026 年 7 月；事实表是否已经同步到 7 月只影响本月实际值、开户、版本、交付等指标，不会把上一事实月显示为“本月”。

前端 KPI 默认日期范围同样按中国时区当前自然月生成，完整自然月范围按 100% 口径计算；手动查看历史完整月份时也不会因月份天数差异缩放 KPI。

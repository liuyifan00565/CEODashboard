...
更新内容:
- 公司月度 `structure` 只向经营总览半环提供特殊渠道展示金额；该金额已经包含在 `total` 中，不再叠加进入主 KPI、自然年回款选源、退款分摊或成本/ROI。
- `total` 继续作为权威公司毛回款，`channel` 继续仅作为四个经营渠道的分摊权重，三种记录层级在数据库约束中互斥并按月保持自然唯一。
- 半环会从四渠道图形份额中按比例拆出特殊渠道，图形合计仍等于权威回款；公司月度导入默认保留订单事实，订单继续用于人员和成交来源下钻。

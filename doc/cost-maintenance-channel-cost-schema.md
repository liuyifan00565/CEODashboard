...
更新内容: 成本维护改为按月份和渠道合并成本表与统一退款视图，退款无对应成本行也不丢失，人力成本 NULL 仍表示未配置。
...
- 渠道总成本 = 运营成本 + 人力成本。
- 毛回款来自 `v_revenue_gross_canonical`，退款来自 `v_revenue_refund_canonical`；净实际回款 = 毛回款 - 退款。退款可以大于当月毛回款，此时净实际回款允许为负数。
...
`refund_amount_yuan` 与成本字段共用“月份 + 渠道”维护粒度，但统计语义独立：它只从毛回款中扣减，不能并入运营成本、人力成本或公司总成本。读取时以 `v_revenue_refund_canonical` 为统一入口，视图的 `source_kind` 用于标识维护数据或真实业务数据来源。
...
数据库完整性迁移 `scripts/migrate_database_integrity.sql` 会把退款字段统一为 `DECIMAL(18,2)`，并建立 `v_revenue_gross_canonical` 与 `v_revenue_refund_canonical`。成本维护读取毛回款时只使用前者；读取成本与退款时，以“月份 + 渠道”的全键集合并 `biz_channel_cost_monthly` 和后者。因此即使某月某渠道没有成本行，对应退款仍会返回；此时 `labor_amount_yuan` 保持 `NULL`，仍表示人力成本未配置。
...
更新内容: 停用或无法归属到当前启用渠道的历史回款、退款和成本仍进入“全部渠道”总计，但不生成可编辑明细行。

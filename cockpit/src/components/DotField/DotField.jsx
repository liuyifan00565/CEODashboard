/*
 更新时间: 2026-06-24 16:26:04
 更新内容: 修复页面滚动后 DotField 鼠标响应位置偏移，改用当前 fixed 容器的 client 坐标；
          鼠标停留时保持 hover/bulge 响应，让点阵跟手。
*/
...
const DotField = memo(({
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      const targetEngagement = m.active ? Math.max(Math.min(m.speed / 5, 1), 0.22) : 0;
      engagement.current += (targetEngagement - engagement.current) * 0.14;

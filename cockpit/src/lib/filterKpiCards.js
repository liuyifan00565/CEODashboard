...
function recoverySectionKeywords(cardKey) {
  if (cardKey === 'year') {
    return ['年度节奏', '明细 >', '年度累计回款', '年度目标', '年度完成率', '时间进度', '已完成 53.8%', '剩余 46.2%', '下半年月均需完成 447 万', '渠道完成情况'];
  }
  if (cardKey === 'month') {
    return ['2026年6月经营进度', '查看近期明细', '本月回款', '月度完成率', '时间进度', '领先 7.1%', '风险渠道', '完成率 70%', '影响月度缺口 36万', '渠道完成情况'];
  }
  return [];
}

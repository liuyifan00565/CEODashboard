...
function recoverySectionKeywords(cardKey) {
  if (cardKey === 'year') {
    return ['年度节奏', '查看年度拆解', '年度累计回款', '年度目标', '年度完成率', '时间进度', '已完成 53.8%', '剩余 46.2%', '下半年月均需完成 447 万', '渠道目标完成结构'];
  }
  if (cardKey === 'month') {
    return [`${META.monthLabel}经营进度`, '查看近期明细', '本月回款', '月度完成率', '时间进度', '领先 7.1%', '风险渠道', '完成率 70%', '渠道目标完成结构'];
  }
  return [];
}

...
function recoverySectionKeywords(cardKey) {
  if (cardKey === 'year') {
    return ['年度回款总览', '查看年度拆解', '年度累计回款', '年度回款结构', '年度目标', '年度完成率', '年目标完成率', '剩余目标', '年度目标进度', '时间进度 50.0%', '高于线性进度 3.8pp', '后续月均需完成 447万', '经营情况', '年度回款', '实际', '目标', '缺口', '风险'];
  }
  if (cardKey === 'month') {
    return [`${META.monthLabel}经营进度`, '查看近期明细', '本月回款', '本月回款结构', '月度完成率', '经营情况', '实际回款', '目标回款', '实际', '目标', '缺口', '超额完成', '风险', '完成率 70%'];
  }
  return [];
}

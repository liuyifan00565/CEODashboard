/* 更新时间: 2026-07-14 19:06:34 CST  更新内容: 回归锁定特殊渠道只拆分 total 内部图形份额，不重复抬高半环总回款和完成率。 */
...
test('keeps structure-only slices inside the authoritative channel total', () => {
  assert.doesNotMatch(source, /const totalRecovered = \[\.\.\.rows, \.\.\.structureRows\]/);
  assert.match(source, /const totalRecovered = rows\.reduce/);
  assert.match(source, /const structureRecovered = Math\.min\(rawStructureRecovered, Math\.max\(0, totalRecovered\)\);/);
  assert.match(source, /const channelDisplayScale = totalRecovered > 0 \? \(totalRecovered - structureRecovered\) \/ totalRecovered : 0;/);
  assert.match(source, /value: row\.pieRecovered,/);
});

...
更新内容: 交付看板改为 KPI 后六项月度对比带、渠道阶段概览与 58/42 同屏执行明细；数据口径和联动保持不变。
...
交付看板已重构为“售前试用转化与配置负载”页面，阅读顺序为 4 张核心指标卡、六项本月 / 上月对比带、当前试用渠道与阶段风险概览，以及单一“交付执行明细”玻璃工作台。执行明细在桌面端按 58% / 42% 同屏展示渠道 / 团队成熟队列转化和配置人员负载，在窄屏自动上下堆叠。当前售前试用字段尚未进入数据库快照，`src/data/presaleTrialDelivery.js` 继续集中保留 2026-07、2026-06 与比较基线演示快照，仅供测试或显式注入加载器使用；运行态默认停用演示快照，未接入完整真实快照时展示空态，不回退使用旧 `deliveryRows`。

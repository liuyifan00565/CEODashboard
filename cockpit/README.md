...
月度 `点击查看近期明细 ›` 与年度 `点击查看年度拆解 ›` 分别位于对应回款结构半环图右下方；两个入口复用现有 KPI 二级弹窗，保留透明大点击热区，点到文字周边也能进入二级页面，但视觉上不做成胶囊按钮。年度入口会向半环内部上收，年度进度条层级高于入口热区，避免透明点击层覆盖进度条。首页回款结构由总览卡内半环承担，不恢复旧首页两张本月/本年半环长卡。

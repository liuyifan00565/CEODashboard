...
年度回款总览以 `/api/dashboard-data` 返回的 `kpi.yearRecovered` 和 `kpi.yearTarget` 为权威值；卡片参考月度主卡展示年度累计回款大数字、年度回款结构半环和年度经营情况列表，并在年度左侧事实区两个胶囊下方用薄年度目标进度条表达 `年度累计回款 / 年度目标`、进度条和完成率；桌面端进度条向右延展到中间结构区分界线内侧，避免挤占右侧经营情况。
...
顶部 `点击查看近期明细 ›` 和年度卡右上角 `点击查看年度拆解 ›` 入口复用现有 KPI 二级弹窗；入口保留较大的可点击热区，但视觉上不再做成胶囊按钮。首页回款结构由总览卡内半环承担，不恢复旧首页两张本月/本年半环长卡。

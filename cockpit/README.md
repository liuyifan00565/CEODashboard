...
更新内容: 同步首页开户数 sparkline、月度趋势风险展示和版本情况四主版本表格占比口径说明。
...
首页版本情况面板保留左侧半环图和数量/金额切换，右侧使用列表型表格展示四个主版本：启航版、卓越版、至尊版、定制版。试用版和增购包仍保留在 mock 数据中，但不进入首页右侧版本表。

版本表固定展示 `版本 / 单价 / 套数 / 占比 / 回款 / 环比` 六列，其中 `占比` 按四个主版本套数合计计算，即 `该版本套数 / 四个主版本套数总和`。每行可点击或使用 Enter/Space 打开原版本二级弹窗。当前数据仍为 `src/data/mock.js` 内的演示数据，后续接入真实口径时应替换为 BI/API 返回的版本维度数据。

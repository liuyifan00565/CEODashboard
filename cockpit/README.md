...
月度 `点击查看近期明细 ›` 位于本月回款结构半环图右下方；年度 `点击查看年度拆解 ›` 位于年度回款结构半环图右下区域、年度进度条上方。两个入口复用现有 KPI 二级弹窗，保留横向透明点击热区，点到文字周边也能进入二级页面，但视觉上不做成胶囊按钮。入口热区保持扁平，年度入口与进度条保持间距，避免透明点击层覆盖或相交进度条。首页回款结构由总览卡内半环承担，不恢复旧首页两张本月/本年半环长卡。

两张回款总览卡保持紧凑信息密度，月度与年度回款结构半环使用相同尺寸：桌面端图区高度为 218px，图表宽度使用 380px 到 480px 的响应范围；低高度桌面档统一为 208px 和 360px 到 440px。下方继续保留原有业务拆解模块：月度经营趋势、开户数、总投入费比、版本情况和交付面板，用于解释经营结果背后的趋势、成本、产品和交付状态。

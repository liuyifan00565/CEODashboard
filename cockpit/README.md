...
左侧菜单下方有一个透明背景 AI 小人入口，运行态动作使用 `public/mascot-actions/` 下同源同尺寸的福客 AI 独立稳定帧，包括待机、挥手、指引、说话、思考、提醒、庆祝和点击，避免切换到尺寸不一致、人物不完整或带无意义符号的旧动作图；如果 `public/live2d/` 中放入已授权的 Cubism Core 与 `fuxiaoke.model3.json`，则会在保持 sprite 兜底的同时启用可选 Live2D 渲染层。未放入本地福小客 Live2D 模型时，不加载外部样例人物，页面继续显示项目内福客 AI 形象。小人默认待机，鼠标悬停会挥手并跟随指针，点击后打开 AI 经营分析对话框。Live2D 资产检测、降级和授权边界见 [`../doc/mascot-live2d.md`](../doc/mascot-live2d.md)。

...
左侧菜单下方有一个透明背景 AI 小人入口，默认使用 `public/mascot-actions/mascot-idle-fuke-rich.png` 的 imagegen 福客 AI 富帧待机循环，并保留其它 `public/mascot-actions/` 福小客动作帧；如果 `public/live2d/` 中放入已授权的 Cubism Core 与 `fuxiaoke.model3.json`，则会在保持 sprite 兜底的同时启用可选 Live2D 渲染层。未放入本地福小客 Live2D 模型时，不加载外部样例人物，页面继续显示项目内福客 AI 形象。小人默认待机，鼠标悬停会挥手并跟随指针，点击后打开 AI 经营分析对话框。小人会按当前场景在原位置切换动作或形象：`public/assets/mascot/ceo-mascot-kpi-guide.png` 用于引导，`ceo-mascot-report-presenter.png` 用于汇报和分析，`ceo-mascot-risk-alert.png` 用于预警，`ceo-mascot-target-achieved.png` 用于达成庆祝。Live2D 资产检测、降级和授权边界见 [`../doc/mascot-live2d.md`](../doc/mascot-live2d.md)。
...
- `public/live2d/`：福小客 Live2D 模型目标目录；放入 `live2dcubismcore.min.js` 和 `fuxiaoke/fuxiaoke.model3.json` 后，左下角小人会优先使用 Live2D，缺失或授权未确认时自动降级到 sprite

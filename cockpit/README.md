...
更新内容: 同步首页开户数小卡片可点击展开本月目标完成二级弹窗的说明。
...
首页本年目标完成情况卡片下方新增 `本月开户数` 和 `今日开户数` 两张低高度小卡片，当前读取 `src/data/mock.js` 中的 `OPENING_ACCOUNT_METRICS` 虚拟演示数据，并展示相对上月/昨日的涨幅趋势。两张小卡片点击后复用本月目标完成情况的二级弹窗，用于沿用现有销售维度和年/月/日筛选下钻体验。后续接入真实接口时，应由 BI/API 返回开户数及对应对比周期涨幅。

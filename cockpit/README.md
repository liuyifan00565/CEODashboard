...
更新内容: 同步经营总览顶部三段融合结构，并说明月度趋势、开户投入、版本情况和交付面板已恢复到下方业务拆解区。
...
经营总览首页顶部改为三段经营故事流：`经营进度总览`、`年度节奏`、`渠道完成情况`。本月作为主视角展示 `486万` 本月回款、`83.8%` 月度完成率、`94万` 缺口和风险渠道 `线下华东 70%`；年度只作为节奏背景展示年度累计、年度目标、年度完成率、年度缺口和轻量趋势线。

年度节奏以 `src/data/mock.js` 中的 `KPI.yearRecovered` 和 `KPI.yearTarget` 为权威值；轻量趋势线只表达 `1月 → 6月 → 12月目标` 的距离，不新增第二套年度累计口径。经营判断固定展示：“当前年度进度低于理想节奏，需重点关注线下华东回款恢复。”

`渠道完成情况` 只保留一组渠道列表，右上角可在 `本月 / 年度` 间切换，默认本月。每行展示渠道名称、进度条、完成/目标、年度贡献和状态；线下华东使用低饱和风险色标记 `需关注`，点击渠道行仍可展开人员完成明细弹窗。

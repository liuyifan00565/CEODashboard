...
更新内容: 交付执行明细支持点击渠道或团队后仅查看对应渠道配置人员完成情况，初始状态不再展示人员总览。

更新时间: 2026-07-14 15:30:13 CST
更新内容: 本地联调通过显式开关启用 2026-06、2026-07 售前试用演示快照，页面同步标注非生产数据，生产默认关闭。
...
交付看板已重构为“售前试用转化与配置负载”页面，阅读顺序为 4 张核心指标卡、六项本月 / 上月对比带、当前试用渠道与阶段风险概览，以及单一“交付执行明细”玻璃工作台。执行明细在桌面端按 58% / 42% 同屏展示渠道 / 团队成熟队列转化和所选渠道配置人员完成情况，在窄屏自动上下堆叠。左侧渠道和团队行始终完整显示，点击左侧行、上方环图或渠道图例会高亮对应渠道，并让右侧只显示该渠道固定归属人员；未选择渠道时右侧显示操作提示，不展示人员总览。当前售前试用字段尚未进入数据库快照，`src/data/presaleTrialDelivery.js` 继续集中保留 2026-07、2026-06 与比较基线演示快照，人员通过 `staffLoadsByChannel` 按渠道分组，渠道行同时记录未配置人数。本地 Docker 通过 `VITE_ENABLE_PRESALE_TRIAL_DEMO=true` 显式启用并在页面标注“演示快照 · 非生产数据”；生产环境默认关闭，未接入完整真实快照时展示空态，且不回退使用旧 `deliveryRows`。

核心转化率按同一批进入试用并到达观察点的客户计算；执行明细左侧的渠道 / 团队转化按上期启动且完成观察窗的成熟队列计算，两种 cohort 不共用分母。风险卡区分临近到期与重点超期，阶段“已超期”保留全部超期客户，未配置负责人和预计金额缺失属于可重叠的数据治理提醒。执行明细右侧的配置人员负载以 14 单为建议上限：低于 80% 为正常、80% 至 100% 为偏高、超过 100% 为超负荷。完整数据、计算和后端字段缺口见 [`../doc/presale-trial-delivery-dashboard.md`](../doc/presale-trial-delivery-dashboard.md)。

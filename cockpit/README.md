...
更新内容: 同步年度节奏最终版三项核心指标、单行辅助说明和轻量明细入口说明。
...
年度节奏以 `src/data/mock.js` 中的 `KPI.yearRecovered` 和 `KPI.yearTarget` 为权威值；卡片只将 `年度累计回款 3,120万`、`年度完成率 53.8%`、`年度缺口 2,680万` 作为三项核心指标，`时间进度 50.0% · 剩余 6 个月，月均仍需完成 536 万` 降级为辅助说明。趋势图展示 `1-6月累计实线 + 6-12月目标虚线`，仅突出 `1月 437万`、`6月 3,120万` 和 `12月目标 5,800万`。经营判断固定展示：“当前年度完成率略高于时间进度，但线下华东连续低于目标，需优先恢复渠道回款。”
...
顶部 `查看近期明细` 和年度卡右上角 `明细 >` 入口复用现有 KPI 二级弹窗，不恢复旧首页两张本月/本年半环长卡；旧半环结构只作为明细弹窗和后续下钻能力保留。

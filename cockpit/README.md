...
月度 `点击查看近期明细 ›` 位于本月回款结构半环图右下方，桌面三栏布局下首字“点”对应饼图右半区起点标线、底边对齐线下华东行底边；年度 `点击查看年度拆解 ›` 位于年度回款结构半环图右下区域、年度进度条上方。两个入口复用现有 KPI 二级弹窗，保留横向透明点击热区，点到文字周边也能进入二级页面，但视觉上不做成胶囊按钮。入口热区保持扁平，年度入口与进度条保持间距，避免透明点击层覆盖或相交进度条。首页回款结构由总览卡内半环承担，不恢复旧首页两张本月/本年半环长卡。

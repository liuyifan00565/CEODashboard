...
更新内容: 记录 AI 分析入口新增页面文字悬浮千问短气泡，服务端提供 /api/ai/hover-cue 接口。
...
左侧菜单下方有一个透明背景 AI 小人入口，入口使用 3D 舞台承载 `public/ai-mascot-transparent.png` 福小客形象；默认待机，鼠标悬停会挥手并跟随指针，点击后打开 AI 经营分析对话框。对话框会把当前页面的 KPI、销售维度、ROI、产品版本、续费率和月度趋势数据快照发到服务端 `/api/ai/analyze`。服务端再调用阿里云百炼 OpenAI-compatible Chat Completions 接口，并把通义的流式文本结果转发给前端。

页面还支持文字悬浮 AI 气泡：鼠标停留在任意可读页面文字上约 650ms 后，前端会提取该处文字和当前页面数据快照，请求服务端 `/api/ai/hover-cue`。服务端用同一组 DashScope 环境变量调用千问，返回一句适合显示在福小客头顶气泡里的经营提示。前端会缓存同一页面上下文下的同一段文字，避免鼠标频繁移动造成重复请求。
